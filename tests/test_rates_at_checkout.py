...
class TestRatesAtCheckout:
    ...
    def _delete_all_service_groups(self, api: shippo.Shippo):
        service_groups = api.service_groups.list(request={})
        assert service_groups is not None

        for sg in service_groups:
            api.service_groups.delete(service_group_id=sg.object_id)
    ...
    def test_rates_at_checkout(self, api: shippo.Shippo):
        ...
        available_service_levels = [
            ServiceLevelUPSEnum.UPS_GROUND,
            ServiceLevelUPSEnum.UPS_NEXT_DAY_AIR_SAVER,
        ]
        service_levels = [
            ServiceGroupAccountAndServiceLevel(
        ...
        service_group = api.service_groups.create(
            request=ServiceGroupCreateRequest(
                name="UPS shipping",
        ...
        assert live_rates is not None
        assert live_rates.results is not None
