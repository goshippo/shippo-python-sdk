...
def test_orders_list_orders_successfully_get_all_orders():
    s = shippo.Shippo(
...
def test_orders_create_order_successfully_create_order():
    s = shippo.Shippo(
        api_key_header="<YOUR_API_KEY_HERE>",
    )
...
def test_orders_get_order_successfully_get_an_order():
    s = shippo.Shippo(
        api_key_header="<YOUR_API_KEY_HERE>",
    )
    ...
    res = s.orders.get(order_id='1267ab11165b44ff993634b28e5d9ce5')
    assert res is not None
    assert res is not None
    assert res == components.Order(
        currency='USD',
