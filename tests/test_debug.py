...
class TestDebugClient:
    ...
    def debug_api(self, debug_session: DebugSession):
        return shippo.Shippo(
            client=debug_session, api_key_header="", server_url=BASE_URL
        )
    ...
    def invoke_api_and_verify_request_and_response_are_set(
        ...
        assert debug_session.last_request.url.params.get("page") == str(page)
    ...
    def test_should_record_request_and_response(
        ...
        self.invoke_api_and_verify_request_and_response_are_set(
            debug_session, debug_api
        )
    ...
    def test_should_record_request_and_response_across_async_tasks(
        ...
        async def run_async():
            await asyncio.gather(
                self.async_invoke_api_and_verify_request_and_response_are_set(
                    debug_session, debug_api
                ),
                self.async_invoke_api_and_verify_request_and_response_are_set(
                    debug_session, debug_api
